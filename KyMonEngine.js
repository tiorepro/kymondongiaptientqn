...
(function (root, factory) {
  ...
  const SYS_HADO = {
    ...
    CUNG_META: {
      ...
      4: { ten: 'Tốn',  huong: 'Tây Nam',   hanh: 'Mộc' }, 
  ...
  function tinhBatThan(cungDichSao, isDuong, isPhiBan, cungKyTrung, sys) {
    ...
    if (isPhiBan) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        bt[current_cung] = sys.THAN_ORDER[i];
      }
    } else {
  ...
  function tinhMonBucChe(batMon, sys) {
    ...
    for (let c = 1; c <= 9; c++) {
      ...
      const hm = NGU_HANH_MON[mon] || '', hc = sys.CUNG_META[c]?.hanh || '', tenCung = sys.CUNG_META[c]?.ten || '';
  ...
  function tinhCachCuc(tb, db, bm, bt, tcb, sys) {
    ...
    for (let cung = 1; cung <= 9; cung++) {
      ...
      const hc = sys.CUNG_META[cung]?.hanh || '', hs = NGU_HANH_SAO[sao] || '', hm = NGU_HANH_MON[mon] || '';
  ...
  function lapBanKyMon(dateStr, options = {}) {
    ...
    return {
      ...
      heThong: heThong, // Lưu lại hệ thống đang chạy
      sys: sys,         // Lưu lại toàn bộ config để LogicLuanQue.js đọc
  ...
  return { byDatetime: lapBanKyMon, version:'6.0.0-Unified', utils: { NGU_HANH_CAN, NGU_HANH_SAO, NGU_HANH_MON, tuongSinh, tuongKhac } };
}));

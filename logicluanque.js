...
// Giữ nguyên 100% hướng dẫn gốc + Tích hợp thuật toán luận giải chuyên gia Kỳ Môn (Nguyệt Lệnh, Đối Xung, MVC, Tinh Môn, Ẩn Can, Môn Cung)
...
(function () {
  ...
  const MAP_MON = {
    'Khai':'Kim','Hưu':'Thủy','Sinh':'Thổ','Thương':'Mộc',
    'Đỗ':'Mộc','Cảnh':'Hỏa','Tử':'Thổ','Kinh':'Kim'
  };
  ...
  const OPPOSITE_PALACE = { 1:9, 2:8, 3:7, 4:6, 6:4, 7:3, 8:2, 9:1, 5:5 };
  ...
  // ============================ EXPERT LOGIC: NGUYỆT LỆNH & ĐỐI XUNG ============================
  ...
  // ============================ PHÂN TÍCH THIÊN-ĐỊA BÀN NỘI TẠI ============================
  ...
  function analyzeMonCungRelation(palace, chart) {
    ...
    const gateHanh = MAP_MON[gateNorm];
  ...
  // ============================ CORE ANALYZER (WITH EXPERT INJECTIONS) ============================
  function analyzePalaceFull(palace, roleName, chart) {
    ...
    // Tính Lệnh Tháng cho Môn và Sao
    let finalGateScore = baseGateScore;
    let vtGateInfo = '';
    if (gate && monthHanh) {
      const gHanh = MAP_MON[normalizeGateName(gate)];
    ...
    if (gate) {
      ...
      // --- V2 INJECTION: Phân tích Môn-Cung tương tác ---
    ...
    // --- V2 INJECTION: Phân Tích Tinh Môn Tổ Hợp ---
  ...
  TOPIC_ANALYZERS[2] = function(chart) {
    ...
    if (pKhai && pDay) {
      ...
      if (rel.rel === 'a_sinh_b') { html += UI.note('🟢 Sự nghiệp sinh trợ bản thân: Công việc phù hợp năng lực, dễ thăng tiến.', 'good'); score += 2; }
      else if (rel.rel === 'b_sinh_a') { html += UI.note('🟡 Bản thân sinh cho công việc: Phải bỏ công sức nhiều nhưng có kết quả.', 'neutral'); score += 1; }
      else if (rel.rel === 'a_khac_b') { html += UI.note('🔴 Công việc áp chế bản thân: Stress cao, môi trường độc hại, khó phát triển.', 'bad'); score -= 2; }
      else if (rel.rel === 'b_khac_a') { html += UI.note('🟡 Bản thân khắc công việc: Kiểm soát được công việc nhưng hao tổn sức lực.', 'neutral'); score -= 1; }
    }
  ...
  TOPIC_ANALYZERS[3] = function(chart) {
    ...
    if (pKhai && pDay) {
      const rel = getRelationBetweenPalaces(pKhai, pDay, chart);
      html += UI.line('Khai Môn ↔ Bản thân', rel.label, rel.rel.includes('sinh') ? 'good' : rel.rel.includes('khac') ? 'bad' : 'neutral');
      if (rel.rel === 'a_sinh_b') { html += UI.note('🟢 Khai Môn sinh bản thân: Quyết định chính thức ủng hộ — khả năng thăng chức CỰC CAO.', 'good'); score += 4; }
      else if (rel.rel === 'dong_hanh') { html += UI.note('🟢 Tỷ hòa: Quyết định thuận chiều bản thân — thăng chức khả thi cao.', 'good'); score += 2; }
      else if (rel.rel === 'b_sinh_a') { html += UI.note('🟡 Bản thân sinh cho quyết định: Cần phải tự chứng minh thêm, chưa có kết luận.', 'neutral'); score += 1; }
      else if (rel.rel === 'a_khac_b') { html += UI.note('🔴 Khai Môn khắc bản thân: Quyết định đi ngược lại mong muốn — thăng chức thất bại hoặc vị trí mới nhiều thử thách.', 'bad'); score -= 3; }
      else if (rel.rel === 'b_khac_a') { html += UI.note('🟡 Bản thân khắc Khai Môn: Bản thân muốn nhưng tổ chức chưa sẵn sàng — cần vận động thêm.', 'neutral'); score -= 1; }
    }
    ...
    score += supportScore;
    if (supportScore >= 3) { html += UI.verdict('✅ Hệ cấp trên đang ủng hộ mạnh mẽ — lực đẩy thăng chức lớn.', 'good'); }
    else if (supportScore <= -2) { html += UI.verdict('❌ Hệ cấp trên đang phản đối hoặc cản trở — lực cản thăng chức lớn.', 'bad'); }
  ...
  TOPIC_ANALYZERS[4] = function(chart) {
    ...
    html += UI.note(`🧭 Người dẫn đường ở hướng: ${direction} — Cung ${cungName}. Hãy tìm kiếm theo hướng này.`, 'info');
    ...
    html += UI.section('💡 Gợi Ý Tiếp Cận');
    html += UI.note(`📍 Tìm theo hướng ${direction}. Thiên Phụ đang ở Cung ${cungName} (${palaceElement(pPhu.cung, chart)}). Nếu đặt phòng họp hoặc gặp mặt, ưu tiên không gian phía ${direction}.`, 'info');
  ...
  TOPIC_ANALYZERS[5] = function(chart) {
    ...
    if (pDo && pDay) {
      ...
      if (rel.rel === 'a_sinh_b') { html += UI.note('🟢 Công ty mới sinh trợ bản thân: Môi trường mới cực kỳ phù hợp, phát triển nhanh, đãi ngộ tốt.', 'good'); score += 3; }
      else if (rel.rel === 'b_sinh_a') { html += UI.note('🟡 Bản thân cống hiến nhiều cho công ty mới: Sẽ được trọng dụng nhưng phải bỏ ra nhiều.', 'neutral'); score += 1; }
      else if (rel.rel === 'a_khac_b') { html += UI.note('🔴 Công ty mới khắc bản thân: Văn hóa công ty và phong cách làm việc không tương thích — nguy cơ thất bại cao.', 'bad'); score -= 3; }
      else if (rel.rel === 'b_khac_a') { html += UI.note('🟡 Bản thân khắc công ty: Vị trí mới nhiều áp lực, nhưng có thể phát huy quyền lực cao.', 'neutral'); score -= 1; }
      else if (rel.rel === 'dong_hanh') { html += UI.note('🟢 Tỷ hòa: Văn hóa công ty tương đồng — dễ hòa nhập, phát triển ổn định.', 'good'); score += 2; }
    }
  ...
  TOPIC_ANALYZERS[6] = function(chart) {
    ...
    if (pKhai && pDay) {
      const rel = getRelationBetweenPalaces(pKhai, pDay, chart);
      if (rel.rel === 'a_sinh_b') { html += UI.note('✅ Khai Môn sinh bản thân: ĐẬU — Công ty chủ động và nhiệt tình mời nhận. Rất cát.', 'good'); score += 4; }
      else if (rel.rel === 'b_sinh_a') { html += UI.note('🟡 Bản thân sinh Khai Môn: Cơ hội có nhưng cần thêm nỗ lực thuyết phục — có thể đậu vòng sau.', 'neutral'); score += 1; }
      else if (rel.rel === 'a_khac_b') { html += UI.note('❌ Khai Môn khắc bản thân: RỚT — Yêu cầu vị trí không phù hợp hoặc đã chọn ứng viên khác.', 'bad'); score -= 4; }
      else if (rel.rel === 'b_khac_a') { html += UI.note('🟡 Bản thân khắc Khai Môn: Bạn đủ điều kiện nhưng vị trí tuyển dụng có giới hạn — phụ thuộc vào cạnh tranh.', 'neutral'); score -= 1; }
      else if (rel.rel === 'dong_hanh') { html += UI.note('🟢 Tỷ hòa: Tương đồng với yêu cầu vị trí — cơ hội 60-70%.', 'good'); score += 1; }
  ...
  TOPIC_ANALYZERS[9] = function(chart) {
    ...
    if (pDay) {
      const rel = getRelationBetweenPalaces(pKhai, pDay, chart);
      if (rel.rel === 'a_sinh_b') { html += UI.note('🟢 Khai Môn sinh bản thân: Nghề nghiệp này cực kỳ phù hợp — sẽ phát tài.', 'good'); score += 3; }
      else if (rel.rel === 'a_khac_b') { html += UI.note('🔴 Khai Môn khắc bản thân: Ngành nghề này không phù hợp với bạn — dễ thất bại.', 'bad'); score -= 3; }
  ...
  TOPIC_ANALYZERS[16] = function(chart) {
    ...
    if (!voids.length) {
      ...
      const best = voids[0];
      html += UI.note(`📍 Hướng đỗ xe tốt nhất: ${palaceDirection(best.cung, chart)} (Cung ${palaceName(best.cung, chart)}). Điều hướng về phía đó.`, 'good');
    }
  ...
  TOPIC_ANALYZERS[17] = function(chart) {
    ...
    html += UI.section('⚠️ Rủi Risk Hành Trình');
    if (pBong) {
  ...
  TOPIC_ANALYZERS[18] = function(chart) {
    ...
    if (pAt && pCanh) {
      ...
      if (relCung.rel === 'dong_hanh') {
        html += UI.note('🟢 Hai cung đồng ngũ hành: Vợ chồng hòa hợp, tư duy tương đồng, ít xung đột.', 'good'); score += 2;
      } else if (relCung.rel === 'a_sinh_b') {
        html += UI.note('🟢 Cung Ất sinh Cung Canh: Bên nữ chủ động vun đắp, nâng đỡ bên nam — hôn nhân hòa thuận, bền vững.', 'good'); score += 3;
      } else if (relCung.rel === 'b_sinh_a') {
        html += UI.note('🟢 Cung Canh sinh Cung Ất: Bên nam chủ động vun đắp, nâng đỡ bên nữ — tình cảm thắm thiết, hôn nhân ổn định.', 'good'); score += 3;
      } else if (relCung.rel === 'a_khac_b') {
        html += UI.note('🔴 Cung Ất khắc Cung Canh: Bên nữ khắc chế bên nam — nam thường ở thế bị động, dễ bị kiểm soát, xung đột về quyền lực trong gia đình.', 'bad'); score -= 2;
      } else if (relCung.rel === 'b_khac_a') {
        html += UI.note('🔴 Cung Canh khắc Cung Ất: Bên nam khắc chế bên nữ — nữ dễ bị áp lực, không được tôn trọng, ảnh hưởng đến sức khỏe và tâm lý.', 'bad'); score -= 2;
      }
  ...
  TOPIC_ANALYZERS[22] = function(chart) {
    ...
    if (pKhai && pDay) {
      const rel = getRelationBetweenPalaces(pKhai, pDay, chart);
      if (rel.rel === 'a_sinh_b') { html += UI.note('🟢 Tòa ủng hộ bên này: Phán quyết có lợi — XÁC SUẤT THẮNG KIỆN CAO.', 'good'); score += 4; }
      else if (rel.rel === 'a_khac_b') { html += UI.note('🔴 Tòa bất lợi cho bên này: Phán quyết nghiêng về phía đối phương.', 'bad'); score -= 3; }
      else if (rel.rel === 'b_khac_a') { html += UI.note('🟡 Bên này đang tác động lên tòa: Có thể vận động được nhưng cần thêm bằng chứng.', 'info'); score += 1; }
    }
  ...
  TOPIC_ANALYZERS[24] = function(chart) {
    ...
    if (pLH) { html += UI.note(`📍 Lục Hợp ở hướng ${palaceDirection(pLH.cung, chart)} (Cung ${palaceName(pLH.cung, chart)}): Đây là hướng người lạc BAN ĐẦU đi theo — bắt đầu tìm kiếm từ đây.`, 'info'); }
    if (pDo) { html += UI.note(`📍 Đỗ Môn ở hướng ${palaceDirection(pDo.cung, chart)} (Cung ${palaceName(pDo.cung, chart)}): Hướng người lạc có thể đang ẨN NÁU hoặc di chuyển đến.`, 'info'); }
    if (pHour) { html += UI.note(`📍 Can Giờ (Dụng thần) ở hướng ${palaceDirection(pHour.cung, chart)} (Cung ${palaceName(pHour.cung, chart)}): Vị trí hiện tại nghi vấn — ưu tiên tìm kiếm khu vực này.`, 'info'); }
  ...
  TOPIC_ANALYZERS[25] = function(chart) {
    ...
    if (pHour) {
      ...
      html += UI.note(`📍 Can Giờ (Vật mất) ở hướng ${palaceDirection(pHour.cung, chart)} — Cung ${palaceName(pHour.cung, chart)}: Tìm kiếm theo hướng này trước.`, 'info');
      if (dH.includes('Thái Âm')) { html += UI.note('💡 Mẹo chuyên gia (Thái Âm): Vật bị che khuất. Hãy tìm trong tủ đóng kín, dưới nệm, trong chăn, rèm cửa hoặc nơi thiếu sáng.', 'info'); }
      if (dH.includes('Cửu Địa')) { html += UI.note('💡 Mẹo chuyên gia (Cửu Địa): Vật rớt ở vị trí thấp. Hãy tìm sát mặt đất, dưới gầm bàn/giường, khe hở dưới thấp hoặc tầng hầm.', 'info'); }
      if (dH.includes('Cửu Thiên')) { html += UI.note('💡 Mẹo chuyên gia (Cửu Thiên): Vật ở vị trí cao. Tìm trên kệ, trên nóc tủ, chỗ cao.', 'info'); }
    }
    if (pHuyenVu && pHuyenVu.cung !== pHour?.cung) {
      html += UI.note(`📍 Huyền Vũ ở hướng ${palaceDirection(pHuyenVu.cung, chart)}: Kẻ tình nghi có thể đến từ hoặc ở về hướng này.`, 'info');
    }
  ...
  function runAnalysis(chartArg) {
    ...
    try {
      ...
      const finalHtml = flagHtml + analysisObj.html + UI.verdict(
        `<div style="display:flex;flex-direction:column;width:100%;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:15px; font-weight: bold;">Tổng điểm lực lượng cục diện:</span>
            <span style="font-size:18px; font-weight: bold;">${analysisObj.score > 0 ? '+' : ''}${Math.round(analysisObj.score * 10) / 10} điểm</span>
          </div>
          <div style="font-size:12px;color:#6b7280;margin-top:5px;font-weight:normal;font-style:italic;">
            * Đã tích hợp Logic V2: Môn Cung Sinh Khắc, Lục Nghi Ẩn Can & Tinh Môn Tổ Hợp.
          </div>
         </div>`,

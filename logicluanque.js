...
// Giữ nguyên 100% hướng dẫn gốc + Tích hợp thuật toán luận giải chuyên gia Kỳ Môn (Nguyệt Lệnh, Đối Xung, MVC)
...
(function () {
  ...
  TOPIC_ANALYZERS[25] = function(chart) {
    ...
    if (pHuyenVu && pHuyenVu.cung !== pHour?.cung) {
      html += UI.note(`📍 Huyền Vũ ở hướng ${palaceDirection(pHuyenVu.cung, chart)}: Kẻ
tình nghi có thể đến từ hoặc ở về hướng này.`, 'info');
    }
  ...
  function runAnalysis(chartArg) {
    ...
    try {
      ...
      if (!fn) {
        if (resultDiv) resultDiv.innerHTML = '<div style="color:red;">Lỗi: Không tìm thấy bộ phân tích chủ đề này.</div>';
        return;
      }

...
// Giữ nguyên 100% hướng dẫn gốc + Tích hợp thuật toán luận giải chuyên sâu an toàn (Crash-proof)
...
(function () {
  ...
  // ============================ UI FORMATTING (GIAO DIỆN) ============================
  function h(s) { return s; } // Bắt buộc phải có hàm này để hiển thị biến số vars
  ...
  // Hàm luận giải mặc định
  function defaultDeepAnalyzer(chart, topicId) {
      const dayHS = getStemOf(chart, 'day'), hourHS = getStemOf(chart, 'hour');
      let score = 0;
      let html = section('Thông Số Cốt Lõi (Can Ngày & Giờ)');
      
      const pDay = findPalaceByHeavenlyStem(chart, dayHS);
      const pHour = findPalaceByHeavenlyStem(chart, hourHS);
      
      html += analyzePalaceDeep(pDay, 'Can Ngày (Chủ Thể)', chart);
      html += analyzePalaceDeep(pHour, 'Can Giờ (Sự Việc)', chart);

      if (pDay && pHour) {
          const rel = getRelationPalace(pHour, pDay, chart);
          if (rel.rel === 'a_sinh_b') { html += note('Sự việc sinh Bản thân: Thuận lợi, dễ dàng đạt kết quả tốt.'); score += 2; }
          else if (rel.rel === 'a_khac_b') { html += note('Sự việc khắc Bản thân: Gặp nhiều trở ngại, áp lực lớn.'); score -= 2; }
          else if (rel.rel === 'b_khac_a') { html += note('Bản thân khắc Sự việc: Cần nỗ lực chi phối, vất vả mới thành.'); score += 1; }
      }

      if (pHour && isVoid(chart, pHour)) { html += note('Sự việc (Can Giờ) Không Vong: Dễ bị hủy bỏ, hư ảo.'); score -= 2; }
      return { html, score };
  }

  for (let i = 1; i <= 26; i++) {
      TOPIC_ANALYZERS[i] = function(chart) { return defaultDeepAnalyzer(chart, i); };
  }

  // --- CÁC CHỦ ĐỀ CÓ LOGIC ĐẶC BIỆT ---
  ...
  TOPIC_ANALYZERS[2] = function(chart) {
    ...
    let pThaiTue = null;
    if (chart && chart.thaiTue && chart.thaiTue.cung) {
       pThaiTue = getPalace(chart, chart.thaiTue.cung);
    }
  ...
  function bindUIEvents() {
    ...
    if (select) {
      select.addEventListener('change', function() {
        ...
        if (guideDiv) {
            guideDiv.innerHTML = TOPIC_GUIDES[topicId] ? renderTopicGuide(topicId) : '<div style="color:var(--muted);text-align:center;padding:40px 0;">Chọn chủ đề để xem hướng dẫn</div>';
        }
        runAnalysis();
      });
    }

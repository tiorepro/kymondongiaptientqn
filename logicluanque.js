...
// Giữ nguyên 100% hướng dẫn gốc + Tích hợp thuật toán luận giải chuyên gia Kỳ Môn (Nguyệt Lệnh, Đối Xung, MVC, Tinh Môn, Ẩn Can, Môn Cung, Cung Can, Phân Kim)
...
(function () {
  ...
  const HUONG_CHI_TIET = {
    ...
    'Đông Bắc': { mau: 'Vàng đất/Nâu', loai: 'Ngõ hẻm, góc khuất, gần tường/đất trống', hanh: 'Thổ' },
    'Tây Nam':  { mau: 'Vàng nhạt', loai: 'Khu dân cư yên tĩnh, không gian phẳng, ruộng đất', hanh: 'Thổ' },
    'Đông Nam': { mau: 'Xanh nhạt', loai: 'Khu mua sắm, thông gió tốt, cửa sổ lớn, vườn hoa', hanh: 'Mộc' },
  ...
  function getConfidenceLevel(score, signalCount) {
    const absScore = Math.abs(score);
    if (absScore >= 10 || signalCount >= 15) return { level: 'RẤT CAO', pct: '85-95%', type: 'good' };
    if (absScore >= 6 || signalCount >= 10) return { level: 'CAO', pct: '70-85%', type: 'good' };
    if (absScore >= 3 || signalCount >= 6) return { level: 'TRUNG BÌNH', pct: '55-70%', type: 'warn' };
    return { level: 'THẤP', pct: '< 55%', type: 'bad' };
  }
  ...
  function renderExecutiveSummary(topicId, chart, finalScore, htmlContent) {
    ...
    const signalCount = (htmlContent.match(/margin-bottom:5px|margin:6px 0/g) || []).length;
    const conf = getConfidenceLevel(finalScore, signalCount);
    
    let outlook = '⚪ TRUNG HÒA / CHƯA RÕ RÀNG';
    let bg = '#f3f4f6', color = '#374151', border = '#9ca3af';
    
    if (finalScore >= 5) { outlook = '🟢 RẤT THUẬN LỢI (ĐẠI CÁT)'; bg = '#f0fdf4'; color = '#14532d'; border = '#16a34a'; }
    else if (finalScore >= 2) { outlook = '🟢 KHÁ THUẬN LỢI (CÁT)'; bg = '#ecfdf5'; color = '#166534'; border = '#22c55e'; }
    else if (finalScore <= -5) { outlook = '🔴 BẤT LỢI NẶNG (ĐẠI HUNG)'; bg = '#fef2f2'; color = '#7f1d1d'; border = '#dc2626'; }
    else if (finalScore <= -2) { outlook = '🟠 KHÔNG THUẬN LỢI (HUNG)'; bg = '#fff1f2'; color = '#991b1b'; border = '#ef4444'; }
    ...
    <div style="margin-bottom:15px; border:2px solid ${border}; border-radius:6px; overflow:hidden;">
        ...
        <div style="padding:10px; background:#fff; font-size:13px; color:#4b5563; display:flex; justify-content:space-between; flex-wrap:wrap;">
            ...
            <div style="width:50%;"><strong>Tổng điểm:</strong> <span style="font-weight:bold; color:${finalScore >= 0 ? '#16a34a' : '#dc2626'}">${finalScore > 0 ? '+' : ''}${Math.round(finalScore * 10) / 10}</span></div>
            <div style="width:50%; text-align:right;"><strong>Độ tin cậy:</strong> <span style="font-weight:bold">${conf.level} (${conf.pct})</span></div>
            <div style="width:100%; margin-top:8px; font-size:12px; border-top:1px dashed #e5e7eb; padding-top:8px;">
  ...
  function analyzePalaceFull(palace, roleName, chart) {
    ...
    // --- V2 INJECTION: Cung - Can Tương Tác ---
